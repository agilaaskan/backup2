		...
		if($prdExistID <= 0 ){  
            ...
            if($prdType != 'configurable' && sizeof($configrable_attributes_array) > 0){	
				...
				if($images_updated == 'no'  && $prdType == 'configurable'){
						...
						$image_url = 'https://www.extremesports.com.kw/media/wysiwyg/placeholder.jpg';
...
function GetCsvFile($prdData,$client,$sessionId, $table_prefix,$conn){
	...
	if(file_put_contents("productPriceCsv/".$csvFile,$csvContent)){
		...
		file_put_contents($file,'');
	 	return  array("status" =>'1',"message"=>'https://www.extremesports.com.kw/apis/'.$file,"log_download_url"=>'https://www.extremesports.com.kw/apis/'.$file);
	}else{
...
function GetStockFileProcess($prdData,$client,$sessionId){
		...
		//unlink($newFileName);
		fclose($file);
	 	return  array("status" =>'1',"message"=>'https://www.extremesports.com.kw/apis/'.$logFile,"log_download_url"=>'https://www.extremesports.com.kw/apis/'.$logFile);
...
function callAPIs($par, $table_prefix='',$conn){
	...
	$client = new SoapClient('https://www.extremesports.com.kw/index.php/api/v2_soap/?wsdl=1');
