...
{
    /**
     * @var IndexCollectionFactory|\Mirasvit\Search\Api\Repository\IndexRepositoryInterface
     */
    ...
    public function getContentBlock($index)
    {
        ...
        if (!$block) {
            throw new \LogicException(__('Child block %1 does not exist', $index->getIdentifier()));
        }
    ...
    {
        $minCollectionSize = $this->config->isHideTabLTFiveResults();
        return $minCollectionSize ? (int)Config::MIN_COLLECTION_SIZE : (int)$minCollectionSize;
    }
}
