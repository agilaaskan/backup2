...
class Result
{
    ...
    public function toArray()
    {
        ...
        if (filter_input(INPUT_GET, 'o') != null) {
			$result['isMisspelled'] = true;
        } else {
			$result['isMisspelled'] = false;
        }

		$result['textMisspelled'] 	= $this->htmlEntityDecode(__('Your search returned no results. Did you mean "%1" ?', $result['query']));
        $result['textAll']   		= $this->htmlEntityDecode(__('Show all %1 results →', $result['totalItems']));
        $result['textEmpty'] 		= $this->htmlEntityDecode(__('Sorry, nothing has been found for "%1".', $result['query']));
