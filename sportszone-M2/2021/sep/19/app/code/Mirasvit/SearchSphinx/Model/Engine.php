...
class Engine
{
     ...
     * @param AttributeCollectionFactory $productAttributeCollectionFactory
     *
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'no such index') !== false) {
                    if ($this->config->isSameServer()) {
                        throw new \LogicException(__('Please reset and restart your sphinx daemon to search by a new index'));
                    } else {
                        throw new \LogicException(__('Please generate a new configuration file'
                            . ' and place it in your remote server to search by a new index'));
                    }
                } elseif (strpos($e->getMessage(), 'unknown column') !== false) {
                    throw new \LogicException(__('Please reset and restart your sphinx daemon to apply changes to the search index'));
                } else {
    ...
    public function isAvailable(&$output = '')
    {
        ...
        if ($proceed) {
            ...
            if (in_array($this->port, $ports)) {
                $output .= __('Searchd has already use the following config file(s) : ' . PHP_EOL . implode('-' . PHP_EOL, $configFiles) . PHP_EOL);
                $output .= __('Please make sure you use different sphinx ports for all these instances.');
    ...
    public function makeConfig()
    {
        ...
        if ($this->directory->isWritable($this->basePath)) {
            ...
            if ($this->directory->isExist($this->configFilePath)) {
                throw new \LogicException(__('File %1 is not writable', $this->configFilePath));
            } else {
                throw new \LogicException(__('Directory %1 is not writable', $this->basePath));
            }
        }
