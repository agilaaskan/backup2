...
$good_ids= $column[0];
$sql2 = "SELECT option_id  FROM `eav_attribute_option_value` WHERE `value` LIKE '$good_ids'";
...
if ($cat != ""){
$sql3 = "SELECT option_id FROM `eav_attribute_option` WHERE `option_id` IN ($cat) AND `attribute_id` = 411";
$qu1 = $conn->query($sql3);
$or_id1 = [];
...
$sql4= "SELECT value, COUNT(value) AS `cnt`
    FROM catalog_product_entity_int WHERE attribute_id =411 AND value IN ($cat1)
    GROUP BY value
    ORDER BY `cnt` DESC
    LIMIT 1";
...
$upd = "UPDATE `catalog_product_entity_int` set value = $category_ids3 WHERE `attribute_id` = 411 AND `value` IN ($cat1)";
$qu4 = $conn->query($upd);
...
$del34 = "DELETE  FROM `eav_attribute_option` WHERE `option_id` IN ($cat6) AND `attribute_id` = 411";
